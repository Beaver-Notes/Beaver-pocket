...
const FloatingMenuComponent: React.FC<FloatingMenuProps> = ({
  ...
  const menuItems = [
    ...
    },
    {
      ...
      action: (editor: any) => editor?.chain().focus().setBlackCallout().run(),
    },
    {
      icon: icons.SingleQuotesLIcon,
      ...
      action: (editor: any) => editor?.chain().focus().setBlueCallout().run(),
    },
    {
      icon: icons.SingleQuotesLIcon,
      ...
      action: (editor: any) => editor?.chain().focus().setGreenCallout().run(),
    },
    {
      icon: icons.SingleQuotesLIcon,
      ...
      action: (editor: any) => editor?.chain().focus().setPurpleCallout().run(),
    },
    {
      icon: icons.SingleQuotesLIcon,
      ...
      action: (editor: any) => editor?.chain().focus().setRedCallout().run(),
    },
    {
      icon: icons.SingleQuotesLIcon,
      ...
      action: (editor: any) => editor?.chain().focus().setYellowCallout().run(),
    },
    {
      ...
      action: (editor: any) => editor?.chain().focus().setMathBlock().run(),
    },
    {
      ...
      action: (editor: any) =>
        editor?.chain().focus().setMermaidDiagram().run(),
    },
    {
  ...
  const handleAction = (action: () => void) => {
    ...
    if (slashPosition !== null && text[slashPosition] === "/") {
      const newContent =
        text.slice(0, slashPosition) + text.slice(slashPosition + 1);
  ...
  return (
    ...
    >
      <div className="max-h-40 overflow-y-auto flex flex-col space-y-2 no-scrollbar">
        {/* Map through the menuItems array to generate buttons */}
        {menuItems.map((item, index) => (
          <button
            key={index}
            onClick={() => handleAction(() => item.action(editor))}
          ...
          >
            ...
            <div className="text-left">
              <h3 className="font-medium text-neutral-800 dark:text-[color:var(--selected-dark-text)]">{item.label}</h3>
              <p className="text-sm text-neutral-500 dark:text-[color:var(--selected-dark-text)]">{item.description}</p>
            </div>
          </button>
        ))}
