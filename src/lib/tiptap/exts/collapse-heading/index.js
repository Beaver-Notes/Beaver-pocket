    ...
    return ({ HTMLAttributes, node, editor }) => {
      ...
      return {
        ...
        ignoreMutation(mutation) {
          ...
          // On mobile platforms, be more permissive with content mutations
          if (isMobile) {
            // Allow all mutations within the content DOM on mobile
            if (
              content.contains(mutation.target) ||
              mutation.target === content
            ) {
              return false; // Don't ignore - let TipTap handle it
            }

            // Allow text node mutations on mobile (critical for text splitting)
            if (
          ...
          // For desktop or non-content mutations, use original logic
          return (
        ...
        update: (updatedNode) => {
          // Ensure we're updating the same type and level
          if (
            updatedNode.type !== this.type ||
            updatedNode.attrs.level !== HTMLAttributes.level
          ) {
            return false;
          }
